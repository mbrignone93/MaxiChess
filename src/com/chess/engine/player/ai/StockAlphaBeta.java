...
public class StockAlphaBeta extends Observable implements MoveStrategy {
    ...
    public Move execute(final Board board) {
        ...
        for (final Move move : MoveSorter.EXPENSIVE.sort((board.currentPlayer().getLegalMoves()))) {
            ...
            if (moveTransition.getMoveStatus().isDone()) {
                ...
                final String quiescenceInfo = " " + score(currentPlayer, highestSeenValue, lowestSeenValue) + " Quiescencia: " +this.quiescenceCount;
                s = "\t" + toString() + "(" +this.searchDepth+ "), Media: (" +moveCounter+ "/" +numMoves+ ") " + move + ", Mejor Movimiento:  " + bestMove

                        + quiescenceInfo + ", Tiempo Tomado: " +calculateTimeTaken(candidateMoveStartTime, System.nanoTime());
            } else {
                s = "\t" + toString() + "(" +this.searchDepth + ")" + ", Media: (" +moveCounter+ "/" +numMoves+ ") " + move + " Es Ilegal! Mejor Movimiento: " +bestMove;
            }
        ...
        final String result = board.currentPlayer() + " Selecciona " +bestMove+ " [#Tableros Evaluados = " +this.boardsEvaluated+
                " Tiempo Tomado = " + executionTime /1000+ " Velocidad = " +(1000 * ((double)this.boardsEvaluated/ executionTime));
        System.out.printf("%s Selecciona %s [#Tableros Evaluados = %d, Tiempo Tomado = %d ms, Velocidad = %.1f\n", board.currentPlayer(),
    ...
    private static String score(final Player currentPlayer,
        ...
        if(currentPlayer.getAlliance().isWhite()) {
            return "[Puntos: " +highestSeenValue + "]";
        } else if(currentPlayer.getAlliance().isBlack()) {
            return "[Puntos: " +lowestSeenValue+ "]";
        }
